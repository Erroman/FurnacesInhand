...
{
    class VacuumTimeConverter : IValueConverter
    {
        private App _application;
        private MainWindow _window;
        public VacuumTimeConverter()
        {
        ...
        public object Convert(object value, Type targetType, object parameter, CultureInfo culture)
        {
            ...
            if (Mouse.LeftButton == MouseButtonState.Pressed)
            {
                ...
                {
                    TimeParameterPair tpp = _window.Vacuum_graph_pairs.Where(x => Math.Abs(x.screenPoint.X - (double)value) < 1).Select(x => x).FirstOrDefault();
